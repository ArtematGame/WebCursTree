...
def api():
    try:
        ...
        if method == 'get_balance':
            ...
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM users WHERE login = ?", (session['rgz_login'],))
        ...
        # Можно добавить еще методов по желанию
        elif method == 'users.get_all':
            if 'rgz_login' not in session or not session.get('rgz_is_manager'):
                return jsonify({'error': 'Manager access required'})
            
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute("SELECT id, full_name, login, phone, account, balance, is_manager FROM users")
            ...
            conn.commit()
            conn.close()
            return jsonify({'result': {'success': True, 'account': account}})
